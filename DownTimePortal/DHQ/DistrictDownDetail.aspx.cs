...
public partial class DistrictDownDetail : System.Web.UI.Page
{
     ...
     void Loaddata()
    {
        ...
        string gdata = "select * from Downdetail where (A_End=@dhq or B_End=@dhq) AND LLUP_Date BETWEEN @dt1 AND @dt2";
   ...
   void LoadPOP()
   {
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        if  (DropDownList2.SelectedItem.Text == "Select All"  && DropDownList4.SelectedItem.Text == "Select All")
            {
                gdata = "select * from Downdetail where LLUP_Date BETWEEN @dt1 AND @dt2 AND (A_End=@aend or B_End=@aend) AND Reason like @rsn + '%'";  
            }
        else if (DropDownList3.SelectedItem.Text == "Select All" && DropDownList4.SelectedItem.Text == "Select All")
        {
            gdata = "select * from Downdetail where LLUP_Date BETWEEN @dt1 AND @dt2 AND ((A_End=@aend or A_End=@sq) AND B_End=@bnd) AND Reason like @rsn + '%'";
        }
        else if (DropDownList2.SelectedItem.Text == "Select All")
        {
            gdata = "select * from Downdetail where LLUP_Date BETWEEN @dt1 AND @dt2 AND Reason like @rsn + '%' AND (A_End=@aend or B_End=@aend) AND Bandwidth=@bwth";

        }
        else if (DropDownList2.SelectedItem.Text == Label19.Text && DropDownList4.SelectedItem.Text == "Select All")
        {
            
            gdata = "select * from Downdetail where LLUP_Date BETWEEN @dt1 AND @dt2 AND Reason like @rsn + '%' AND (A_End=@sq or B_End=@bnd)";

        }
        else if (DropDownList2.SelectedItem.Text == Label19.Text )
        {
            
            gdata = "select * from Downdetail where LLUP_Date BETWEEN @dt1 AND @dt2 AND Reason like @rsn + '%' AND (A_End=@sq or B_End=@bnd) AND Bandwidth=@bwth";
        ...
        {
            gdata = "select * from Downdetail where LLUP_Date BETWEEN @dt1 AND @dt2 AND ((A_End=@aend or A_End=@sq) AND B_End=@bnd) AND Bandwidth=@bwth AND Reason like @rsn + '%' ";
