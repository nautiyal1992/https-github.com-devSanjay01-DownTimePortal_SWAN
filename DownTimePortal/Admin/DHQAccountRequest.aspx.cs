...
public partial class Admin_DHQAccountRequest : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            string a= Session["dhqac"].ToString();
             string b= Session["Popac"].ToString();
            string c= Session["shqac"].ToString();
            Loadac(a,b,c);
    ...
    void Loadac(string a, string b, string c)
    {
        string dhqac = a;
        string popac = b;
        string shqac = c;
        ...
        if (dhqac == popac)
        {
            string dhqcmd = "SELECT [Name], [Email], [Mobile], [Gender], [DOB], [SHQ], [DHQ], [POP], [Permission], [AC_Date] FROM [AccountDetail] where DHQ=@dhq";
            Accountdetail.SelectParameters.Add("@dhq", dhqac);
            Accountdetail.SelectCommand = dhqcmd;
            GridView1.DataSource = Accountdetail;
            GridView1.DataBind();

        }
        else if (dhqac == shqac)
        {
            string shqcmd = "SELECT [Name], [Email], [Mobile], [Gender], [DOB], [SHQ], [DHQ], [POP], [Permission], [AC_Date] FROM [AccountDetail] ORDER BY DHQ";
            Accountdetail.SelectParameters.Add("@dhq", dhqac);
            Accountdetail.SelectCommand = shqcmd;
            GridView1.DataSource = Accountdetail;
            GridView1.DataBind();
        }
    }
